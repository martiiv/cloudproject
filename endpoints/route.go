...
func Route(w http.ResponseWriter, request *http.Request) {
	...
	if len(strings.Split(request.URL.Path, `/`)) != 4 { //Splits url in order to get startLocation and endLocation
	...
	if err != nil {
		log.Println("Unable to get request for adress: " + StartAddress + "\n" + err.Error())
	...
	if err != nil {
		log.Println("Unable to get request for adress: " + EndAddress + "\n" + err.Error())
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}
	...
	//For each instruction get manouver and roadnumber
