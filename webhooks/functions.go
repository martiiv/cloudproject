...
func SendNotification(notificationId string) {
	...
	timeString := newTime.Add(+30 * time.Minute).String()
	...
	jsonStart := `{ "text": "` + text + `","attachments": [{`
	jsonMiddleEnd := `"color": "#2eb886",`
	jsonAfterTest := `"author_name": "Roadtrip Planner","title": "Weather",`
	jsonMiddleLink := `"title_link": `
	link := `"http://10.212.141.222:80/weather/` + destination + `",`
	jsonEnd := `"text": "The Weather Forecast for you destination","footer": "The Road trip Companion"}]}`

	jsonData := []byte(jsonStart + jsonMiddleEnd + jsonAfterTest + jsonMiddleLink + link + jsonEnd)
